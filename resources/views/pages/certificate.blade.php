	...
	<section class="mx-auto max-w-7xl space-y-6 py-6">
		...
		<div class="flex flex-col gap-2">
			<div class="flex items-center justify-between">
				<div>
					<h1 class="text-3xl font-bold tracking-tight text-gray-900">Certificados médicos</h1>
					<p class="mt-2 text-sm text-gray-600">Busca y filtra certificados por número, cédula del paciente o rango de fechas.</p>
				</div>
			</div>
		</div>
		...
		<div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
			<form method="get" class="space-y-4">
				...
				<div class="grid gap-4 sm:grid-cols-2">
					<div>
						<label for="search_by_id_card" class="mb-2 block text-sm font-medium text-gray-700">Cédula del paciente</label>
						<input 
							id="search_by_id_card" 
					...
					<div>
						<label for="search_by_certificate_number" class="mb-2 block text-sm font-medium text-gray-700">Número del certificado</label>
						<input 
							id="search_by_certificate_number" 
				...
				<div class="flex gap-2 pt-2">
					<button type="submit" class="inline-flex flex-1 items-center justify-center gap-2 rounded-lg bg-gray-900 px-6 py-2.5 text-sm font-medium text-white shadow-sm transition hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-gray-900 focus:ring-offset-2">
						<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
						Buscar
					</button>
					<a href="{{ url()->current() }}" class="inline-flex items-center justify-center rounded-lg border border-gray-300 px-6 py-2.5 text-sm font-medium text-gray-700 transition hover:bg-gray-50">
						Limpiar
					</a>
				</div>
			</form>
				...
				<div class="border-b border-gray-200 px-6 py-4 bg-blue-50">
					...
					<div class="flex flex-wrap gap-2">
						@if($filters['patient_id_card'])
							<span class="inline-flex items-center gap-1.5 rounded-full bg-blue-100 px-3 py-1 text-xs font-medium text-blue-900">
					...
					<table class="w-full divide-y divide-gray-200">
						<thead class="bg-gray-50">
							<tr>
								...
								<th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-600">Título</th>
								<th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-600">Paciente</th>
								<th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-600">Cédula</th>
								<th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-600">Fecha</th>
						...
						<tbody class="divide-y divide-gray-100">
							@foreach($certificates as $certificate)
								<tr class="hover:bg-gray-50">
									<td class="px-6 py-4 text-sm text-gray-900">{{ $certificate['number'] }}</td>
									<td class="px-6 py-4 text-sm text-gray-900">{{ $certificate['title'] }}</td>
									<td class="px-6 py-4 text-sm text-gray-700">{{ $certificate['patient'] }}</td>
									<td class="px-6 py-4 text-sm text-gray-700">{{ $certificate['id_card'] }}</td>
									<td class="px-6 py-4 text-sm text-gray-700">{{ $certificate['issued_at'] }}</td>
									<td class="px-6 py-4 text-right">
										<div class="flex justify-end gap-2">
											<a href="{{ route('certificates.pdf', ['certificate' => $certificate['number']]) }}" target="_blank" rel="noopener" class="rounded-lg border border-gray-300 px-3 py-1.5 text-xs font-medium text-gray-700 transition hover:bg-gray-50">Ver</a>
										</div>
									</td>
								</tr>
							@endforeach
